...
def get_memory_position(memory_location):
  ...
  for i, (min_arm, max_arm) in enumerate(square_coordinates):
    if memory_location >= min_arm and memory_location <= max_arm:
      ...
      if i == 0:
        coordinates = (half_arm, position_on_arm, square_size)
      elif i == 1:
        coordinates = (position_on_arm, half_arm, square_size)
      elif i == 2:
        coordinates = (-half_arm, position_on_arm, square_size)
      elif i == 3:
        coordinates = (position_on_arm, half_arm, square_size)
  ...
  # for i, (min_arm, max_arm) in enumerate(square_coordinates):
  #   if memory_location >= min_arm and memory_location <= max_arm:
  #     distance = half_arm + memory_location - min_arm - half_arm + 1
  #     break
  # return distance

def manhattan_memory(memory_location):
  return get_memory_position(memory_location)
