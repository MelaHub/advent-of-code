...
class ReallocationTest(unittest.TestCase):
  ...
  @unpack
  def test_number_of_steps(self, init_memory, expected_steps):
    memory_reallocator = MemoryReallocator(init_memory)
    self.assertEquals(memory_reallocator.count_number_steps(), expected_steps)
